...
\title{The Quantum Traveler's Guide: from Wavefunctions to Semiconductors}
\author{Saúl Díaz Mansilla, CID 06070018}
\date{\small (Word count: 303 + 2315)}

\renewcommand{\abstractname}{Research and writing methods}
...
\begin{abstract}
    Research for this essay started with a discussion with friends about the topics they had chosen. One of them mentioned the derivation of the Schrödinger equation using Hamilton-Jacobi theory. This reminded me of a course I had taken at my home university \cite{sabiovera2024}, and I remembered one of the topics I found most interesting from that course: Bloch's theorem and conduction bands. I also remembered a simple statement of the Kronig-Penney model at the PLANCKS competition \cite{plancks2025solutions}, so I decided to follow that derivation.
    ...
    When I had finished the outline, I solved the quantum tunneling problem in various ways to find the simplest one to understand, and did the same for the Kronig-Penney model. Then, I started to think of analogies and experiments to motivate the wavefunction concept. I finally settled on the double slit experiment, as it can be used both as a thought experiment, when thinking about sending particles one by one, and an actual experiment that has been verified. I checked my work with the Feynman Lectures\cite{feynman1965lectures} as they included a nice explanation for this result.

    Figures were made using \textit{Python}, and I sped up the process using GAI to quickly write the main scripts to then modify them myself according to the result I wanted. I also used GAI to quickly find suitable parameters for figure \ref{fig:bands_materials}, which I then verified using other sources.

    Then it was just a matter of putting everything together (figures and equations) and writing. I revised the text various times, checking with the rubric, to find poorly explained sections, and to polish the format and logic. I finally used AI to check for typos.
...
% Brilliant minds managed to explain this paradoxical result by using three ``pills" that lay the foundations of the quantum theory (and they are hard pills to swallow!):
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{two_slit_hist.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{potential_barrier.pdf}
    \caption{Rectangular potential barrier of width $b$ and height $V_0$, approximated by a delta function as $b\to 0$ with constant area $\beta=bV_0$. In blue, we represent the incoming wavefunction of an electron with energy $E<V_0$. Note that the amplitude of the wave is a different concept that the potential, but is represented on the same axis for illustrative purposes.}
...
\end{equation}
Furthermore, by integrating the TISE over a small interval:
\begin{equation}
    -\frac{\hbar^2}{2m}\int_{-\varepsilon}^{\varepsilon} \frac{d^2u}{dx^2}dx + \beta\int_{-\varepsilon}^{\varepsilon} \delta(x)u(x)dx = E\int_{-\varepsilon}^{\varepsilon} u(x)dx
\end{equation}
and taking the limit $\varepsilon\to0$, we see that the RHS goes to zero. Solving the integrals on the RHS and rearranging, we see that its derivative is discontinuous with a jump of:
\begin{equation*}
    u'(0^+)-u'(0^-)=\frac{2m\beta}{\hbar^2}u(0)
\end{equation*}
Fortunately, this is the hardest math we will see for the rest of the essay. If you didn't follow, just note that this gives the equation:
\begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{kp_potential.pdf}
    \caption{Kronig-Penney approximation for the potential of a 1D solid.}
    \label{fig:kp_potential}
\end{figure}

But this is just the same problem we have just solved, with $\beta<0$, and with periodic barriers set apart a distance $a$. Let's look at the barrier set at $x=0$. Assuming each box delimited by the barriers contains one electron, our solution now will look like two wavefunctions superposed with coefficients, one from the left box and one from the right box: $u(x)=Au_\text{left}(x)+Bu_\text{right}(x)$ (we will follow the derivation at [\citen{ashcroft1976solid}] and [\citen{plancks2025solutions}]). Each wavefunction corresponds to an electron going towards the barrier, so:
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{Conduction bands of a 1D crystal. The dotted red lines represent $-1$ and $1$. The allowed energy bands are in blue, and the forbidden gaps in white.}
    \label{fig:bands}
\end{figure}
...
\begin{figure}[H]
     ...
     \begin{subfigure}[b]{0.32\textwidth}
         ...
         \caption{\tiny{Cu (conductor)\cite{latticeconstants}, $qa=0.769$}}
     ...
     \begin{subfigure}[b]{0.32\textwidth}
         ...
         \caption{\tiny{NaCl (insulator)\cite{latticeconstants}, $qa=8.91$}}
     ...
     \begin{subfigure}[b]{0.32\textwidth}
         ...
         \caption{\tiny{Si (semiconductor)\cite{latticeconstants}, $qa=4.64$}}
         \label{fig:bands_silicon}
     \end{subfigure}
     \caption{Conduction bands of various materials using the Kronig-Penney model. The allowed energy bands are in blue, and the forbidden gaps in white.\cite{bandsnote}}
...
Nevertheless, it is still one of the least intuitive theories we have ever developed. It was rejected by many during its development, even by some of its own creators, such as Einstein and Schrödinger.

Today you got a taste of the weirdness of quantum mechanics: the three fundamental facts we have to accept to explain experiments, as well as of its usefulness: how quantum tunneling, one of the most disconcerting resutls of quantum, gives rise to a model of crystals and conductivity unthinkable with classical mechanics. 
% If you accept quantum's weirdness, you will be able to take on a journey towards understanding our most advanced tech and unthinkable with classical physics.
% The journey you have started with this guide will show you how this weird field is the most applicable
...
% - review suggestions and grammar
% - textbook pages on references
